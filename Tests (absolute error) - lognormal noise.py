...
# Defining the noise
noise = np.random.lognormal(0, 1, sample_size)
...
for n in prediction:
    ...
    list_sgd += [sum([np.mean(abs(np.dot(robust_sgd, X[i]) - Y[i])) for i in range(max_trials)]) / max_trials]
    list_rgd += [sum([np.mean(abs(np.dot(stagewise_rgd, X[i]) - Y[i])) for i in range(max_trials)]) / max_trials]
    list_erm += [sum([np.mean(abs(np.dot(empirical_minimization, X[i]) - Y[i])) for i in range(max_trials)])
                 / max_trials]
